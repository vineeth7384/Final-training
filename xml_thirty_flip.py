...
for image_path in imagePaths:
    ...
    frameo = cv2.imread(image_path)
    save = cv2.flip(frameo,1)
    flip_path = "flipped/"+str(args["images"])+str(index)+".jpg"
    cv2.imwrite(flip_path,save)
    frame = cv2.imread(flip_path) 
    ...
    for subject in subjects:
        image = draw_face(flip_path,subject)
        profile = 0
    if(profile == 1) :
